            ...
            entities.push({
                ...
                config: {
                    ...
                    attackSlot: e.combat.skills[0]?.config.type === 'melee'
                        ? {
                            type: 'melee' as const,
                            range: e.combat.skills[0].config.range,
                            damage: e.combat.skills[0].config.damage,
                            cooldown: e.combat.skills[0].config.cooldown,
                            duration: e.combat.skills[0].config.duration,
                        }
                        : {
                            type: 'ranged' as const,
                            range: e.combat.skills[0]?.config.range ?? 10,
                            damage: e.combat.skills[0]?.config.damage ?? 2,
                            cooldown: e.combat.skills[0]?.config.cooldown ?? 0.8,
                            duration: e.combat.skills[0]?.config.duration ?? 0.2,
                            bulletSpeed: e.combat.skills[0]?.config.projectileSpeed ?? 20,
                            bulletKnockback: e.combat.skills[0]?.config.knockbackForce ?? 3,
                            bulletLifetime: e.combat.skills[0]?.config.projectileLifetime ?? 3,
                        },
