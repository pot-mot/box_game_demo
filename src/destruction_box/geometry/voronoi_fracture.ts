...
import type {FragmentData} from '../types/destruction.ts'
...
const computeVoronoiCell = (
    ...
    // Box face constraints: normal·p <= d
    ...
    // Bisector planes from other seeds
    ...
    // Enumerate all 3-plane combinations to find vertices
    const numConstraints = constraints.length
    const rawVerts: Vec3[] = []
    const vertConstraints: number[][] = [] // which planes each vertex lies on
    ...
    // Group vertices by constraint plane
    ...
    // Filter faces with enough vertices
    const facePlaneIndices: number[][] = []
    for (let ci = 0; ci < numConstraints; ci++) {
        if (faceVertSets[ci].length >= 3) {
            // Sort vertices angularly around the face normal
            ...
            // Build reference frame on the plane
...
const computeVolume = (verts: Vec3[], faces: number[][]): number => {
    ...
    for (const face of faces) {
        ...
        for (let i = 1; i < face.length - 1; i++) {
            ...
            // tetrahedron signed volume: (v0 × v1) · v2 / 6
