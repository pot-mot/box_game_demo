...
import {Vec3, World, Body, BODY_TYPES, Box, Material, ContactMaterial, SAPBroadphase} from 'cannon-es'
...
describe('NavSensor 传感器检测', () => {
    ...
    beforeEach(() => {
        ...
        entity = createCharEntity(world, 0, 0, 0)
...
describe('NavFSM 导航状态机', () => {
    ...
    it('9. 高墙 + 左侧通 + nav 开启 → 转向绕行', () => {
        ...
        for (let i = 0; i < 60; i++) {
            ...
            /* 模拟角色沿偏转方向移动 */
            entity.body.position.x += r.dx * entity.config.speed * FIXED_DT
            entity.body.position.z += r.dz * entity.config.speed * FIXED_DT
        }
    ...
    it('14. 畅通路径 + nav 开启 → 持续 navigating', () => {
        ...
        /* 位置应有前进 */
        expect(entity.body.position.x).toBeGreaterThan(0.5)
    })

    it('15. Legacy（nav 关闭）+ 前方墙 → 卡住超时后 idle', () => {
        navCtx.enabled = false
        navCtx.lastPosX = entity.body.position.x
        navCtx.lastPosZ = entity.body.position.z
    ...
    it('16. Legacy（nav 关闭）+ 畅通路径 → 持续前进不被干扰', () => {
        ...
        /* 位置应明显前进 */
        expect(entity.body.position.x).toBeGreaterThan(2)
    })
})
...
const createCharOnBox = (boxTopY: number): CharacterEntity => {
    ...
    const body = new Body({
        mass: 1, type: BODY_TYPES.DYNAMIC, fixedRotation: true,
        position: new Vec3(0, bodyY, 0), linearDamping: 0.2,
    })
    body.addShape(new Box(new Vec3(bw / 2, bh / 2, bd / 2)))
    world.addBody(body)
...
describe('NavSensor 倾斜箱子和箱顶导航', () => {
    ...
    it('44. 前方 30° 倾斜箱子 → 前面命中 → blocked_low（低斜坡可走上去）', () => {
        ...
        const w = new World()
        const entity = createCharEntity(w, 0, 0, 0)
        const result = sensor.sense(entity, 1, 0, navConfig)
