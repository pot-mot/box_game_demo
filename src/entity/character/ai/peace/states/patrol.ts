...
/** 重掷巡逻路点：出生点周边 patrolRadius × 0.8 半径内（建造策略用固定半径 4）。
 * patrol enter / 等待换点 / 卡死恢复三处共用 */
export const rerollWaypoint = (ctx: AIContext): void => {
    const r = isPatrolConfig(ctx.peaceConfig) ? ctx.peaceConfig.patrolRadius * 0.8 : 4
