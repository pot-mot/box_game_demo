...
import {WEAPON_READY_SHOULDER, WEAPON_READY_ELBOW, WEAPON_WALK_ARM_SWING} from '../constants.ts'
...
export const walkingAnim: AnimationHandler = {
    ...
    update: (dt, model, ctx) => {
        ...
        /* 持械臂戒备持握（肩前举 + 肘弯加大 + 随步伐小幅起伏），空手保持自然反相摆臂 */
        const armSwing = -Math.sin(t) * 0.35
        model.rightArmShoulder.rotation.x = ctx.weaponHeld
            ? WEAPON_READY_SHOULDER - Math.sin(t) * WEAPON_WALK_ARM_SWING
            : armSwing
        model.leftArmShoulder.rotation.x = -armSwing

        const armBend = Math.max(0, Math.cos(t)) * 0.2
        model.rightArmElbow.rotation.x = ctx.weaponHeld ? WEAPON_READY_ELBOW : armBend + 0.05
